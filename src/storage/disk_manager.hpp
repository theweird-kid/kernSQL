...
class DiskManager {
  public:
	...
	// if the file size isn't a multiple of PAGE_SIZE, or if the file is non-empty but
	// page 0 isn't a valid META page. On a brand-new file, initializes page 0 as the
	// meta page with an empty freelist. On an existing file, recovers freelist_head_
	...
	// out-of-range page_id or on META_PAGE_ID (the reserved page is not readable
	// through the public API). Does not interpret or validate the bytes read —
	...
	// on an out-of-range page_id or on META_PAGE_ID. Does not clear the page's
	// body — only its header is overwritten to mark it FREE.
  ...
  private:
	...
	// bounds check + rejects META_PAGE_ID; used by the public page-access API so
	// callers can never read/write/deallocate the reserved superblock directly
	[[nodiscard]]
